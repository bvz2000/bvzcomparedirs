...
def compare(file_a_path,
    ...
    :param file_b_checksum:
           If not None, then this will be used as the checksum for file b instead of calculating it. Defaults to None.
    :param single_pass:
           If True, then the two files will be compared using a full checksum of each file. If False, then only the
           first 1K bytes of each file will be checksummed. Only if these bytes match will a second, full checksum of
           both files be done. If file_b_checksum is not None, then single pass will be ignored and a full pass will
           always be run right off the bat.
